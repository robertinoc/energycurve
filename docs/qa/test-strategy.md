...
| `lib/tools/*` + `components/tools/*` (herramienta pública) | **Alto** | La página promete que ningún contenido del archivo sale por red, y la promesa es la razón por la que alguien la usa sin cuenta. Un fetch de más, una server action o un evento de analítica demasiado detallado son el mismo bug. | `e2e/tools.spec.ts` vigila cada request y falla si aparece un título o un artista |
